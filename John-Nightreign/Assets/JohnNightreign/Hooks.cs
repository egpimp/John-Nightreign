...
namespace JohnNightreign
{
    public static class Hooks
    {
        ...
        private static void HealthComponent_TakeDamage(On.RoR2.HealthComponent.orig_TakeDamage orig, HealthComponent self, DamageInfo damageInfo)
        {
            ...
            if (self && self.body && self.body.HasBuff(bdWylderSurvive) && !((damageInfo.damageType.damageType & DamageType.NonLethal) == DamageType.NonLethal))
            {
                ...
                if (effectiveDamage > self.combinedHealth)
                {
                    ...
                    self.body.RemoveBuff(bdWylderSurvive);
                    DotController.RemoveAllDots(self.gameObject);
                    self.body.inventory.GiveItemPermanent(ItemCatalog.FindItemIndex("idWylderPassiveSpent"));
                }
            }
